...
public class MediaPlaybackService extends Service {
    ...
    public static final String PLAYSTATE_CHANGED = "com.neteast.music.playstatechanged";
    public static final String META_CHANGED = "com.neteast.music.metachanged";
    public static final String QUEUE_CHANGED = "com.neteast.music.queuechanged";
    public static final String PLAYBACK_COMPLETE = "com.neteast.music.playbackcomplete";
    public static final String ASYNC_OPEN_COMPLETE = "com.neteast.music.asyncopencomplete";

    public static final String SERVICECMD = "com.neteast.music.musicservicecommand";
    ...
    public static final String TOGGLEPAUSE_ACTION = "com.neteast.music.musicservicecommand.togglepause";
    public static final String PAUSE_ACTION = "com.neteast.music.musicservicecommand.pause";
    public static final String PREVIOUS_ACTION = "com.neteast.music.musicservicecommand.previous";
    public static final String NEXT_ACTION = "com.neteast.music.musicservicecommand.next";

    private static final int TRACK_ENDED = 1;
    private static final int RELEASE_WAKELOCK = 2;
    private static final int SERVER_DIED = 3;
    private static final int FADEIN = 4;
    private static final int PLAY_AFTER_PREPAR = 5;
  ...
  //  private long [] mPlayList = null;
    ...
    private List<DataInfo> mPlayList = new ArrayList<DataInfo>();
    ...
    private String  AfterPearparePlayId =null;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case RELEASE_WAKELOCK:
                    mWakeLock.release();
                    break;
                case PLAY_AFTER_PREPAR:
                	playById(AfterPearparePlayId);
                	break;
                default:
                    break;
            }
    ...
    private void ensurePlayListCapacity(int size) {
        if (mPlayList == null || size > mPlayList.size()) {
            // reallocate at 2x requested size so we don't
            // need to grow and copy the array for every
            // insert
            //long [] newlist = new long[size * 2];
            List<DataInfo> newlist = new ArrayList<DataInfo>();
            int len = mPlayList != null ? mPlayList.size() : mPlayListLen;
            for (int i = 0; i < len; i++) {
                newlist.add ( mPlayList.get(i));
            }
            mPlayList = newlist;
        }
    ...
    private void addToPlayList(List<DataInfo> list, int position) {
    ...
    public void createplaylist(int position){
    	
    	if(mPlayList!=null){
    			mPlayList.clear();
    	}
    	List<DataInfo> list = DataHelpter.getPlaylist();
    	for(int i=0;i<list.size();i++){
    		mPlayList.add(list.get(i));
    	}
    	
    	//mPlayList = DataHelpter.getPlaylist();
    	mPlayListLen = mPlayList.size();
    	/*
    	System.out.println("MediaPlaybackService position :"+position + "list size"+mPlayList.size());
    	if(position > 0){
    		mPlayPos = position;
    	}else mPlayPos = 0;
    	System.out.println("MediaPlaybackService url = " + mPlayList.get(mPlayPos).getUrl());
    	openAsync(mPlayList.get(mPlayPos).getUrl());
    	notifyChange(META_CHANGED);
    	*/
    }
    /**
    ...
    private void openCurrent() {
        synchronized (this) {
            ...
            */
            ...
            stop(false);
            open(mPlayList.get(mPlayPos).getUrl(), false);
           // openAsync(mPlayList.get(mPlayPos).getUrl());
    ...
    public void prev() {
        synchronized (this) {
            ...
            DataInfo info = mPlayList.get(mPlayPos);
            if(info.getSecurity()!=null && info.getSecurity().equals("1")){
            	prev();
            }else{
	       		AfterPearparePlayId = info.getId();
	    		if(!mPlayer.isPreparing()){
	    			stop(false);
	    			openAsync(info.getUrl());
		            notifyChange(META_CHANGED);
		    
	    		}else{
	    			notifyChange(META_CHANGED);
	    			mPlayer.setPlayAfterPrepare(true);
    ...
    private int removeTracksInternal(int first, int last) {
        synchronized (this) {
            ...
            System.out.println(" remove tranck num = " + num + "first" + first + "last" + last);
            /*
            for (int i = 0; i < num; i++) {
                //mPlayList[first + i] = mPlayList[last + 1 + i];
            	System.out.println(" remove tranck i =  " + i );
                mPlayList.add(first+i, mPlayList.get(last+1+i));
            ...
            */
            ...
            if (gotonext) {
                if (mPlayListLen == 0) {
                    ...
                    DataInfo info = mPlayList.get(mPlayPos);
                    AfterPearparePlayId = info.getId();
                    System.out.println("remove Tranck to play mPlayPos = " + mPlayPos + "name = " + mPlayList.get(mPlayPos).getName());
                    /*
                    openCurrent();aaaa
                    if (wasPlaying) {
                        play();
    ...
    public String getName(){
    	synchronized(this){
    		if(mPlayList!=null){
    			String id = AfterPearparePlayId;
    			for(int i=0;i<mPlayList.size();i++){
	    			DataInfo info = mPlayList.get(i);
     ...
     * @param pos The position in the queue of the track that will be played.
    */
    public int playById(String id){   
    	synchronized(this) {
    		System.out.println(" playById  id = " + id);
    		AfterPearparePlayId = id;
    		if(!mPlayer.isPreparing()){
    			stop(false);
	    		for(int i=0;i<mPlayList.size();i++){
	    			DataInfo info = mPlayList.get(i);
    			...
    			System.out.println("mPlayer is preparing");
    			notifyChange(META_CHANGED);
    			mPlayer.setPlayAfterPrepare(true);
    ...
    private class MultiPlayer {
        ...
        public void setDataSourceAsync(String path) {
            try {
            	Log.d("MultiPlayer setDataSource", "111111,mIsInitialized  = " + mIsInitialized );
                mMediaPlayer.reset();
                mMediaPlayer.setDataSource(path);
                Log.d("MultiPlayer setDataSource", "2222222");
                mMediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
                mMediaPlayer.setOnPreparedListener(preparedlistener);
                mMediaPlayer.prepareAsync();
                Log.d("MultiPlayer setDataSource", "3333333");
            } catch (IOException ex) {
                // TODO: notify the user why the file couldn't be opened
                mIsInitialized = false;
                Log.d("MultiPlayer setDataSource", "555555555555");
                DataHelpter.printLog(MediaPlaybackService.this,ex);
                return;
            } catch (IllegalArgumentException ex) {
                // TODO: notify the user why the file couldn't be opened
                mIsInitialized = false;
                Log.d("MultiPlayer setDataSource", "6666666");
                return;
            }
            Log.d("MultiPlayer setDataSource", "77777777");
        ...
        public void setPlayAfterPrepare(boolean value){
        	playAfterPrepare = value;
        }
        ...
        MediaPlayer.OnCompletionListener listener = new MediaPlayer.OnCompletionListener() {
            public void onCompletion(MediaPlayer mp) {
            	...
            	System.out.println("On CompletionListener !!!!");
        ...
        MediaPlayer.OnPreparedListener preparedlistener = new MediaPlayer.OnPreparedListener() {
            public void onPrepared(MediaPlayer mp) {
            	...
            	preparing = false;
            	if(playAfterPrepare){
            		setPlayAfterPrepare(false);
            		mHandler.sendEmptyMessage(PLAY_AFTER_PREPAR);
            	}else{
        ...
        MediaPlayer.OnErrorListener errorListener = new MediaPlayer.OnErrorListener() {
            public boolean onError(MediaPlayer mp, int what, int extra) {
            	preparing = false;
            	System.out.println("On errorListener !!!!");
                switch (what) {
                case MediaPlayer.MEDIA_ERROR_SERVER_DIED:
                    ...
                    Toast.makeText(MediaPlaybackService.this, R.string.playback_failed, Toast.LENGTH_SHORT).show();
                    Log.d("MultiPlayer MEDIA ERROR SERVER DIED", "Error: " + what + "," + extra);
                    return true;
                default:
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
        ...
        public void createplaylist(int position){
        	mService.get().createplaylist(position);
        }
        public void open(long [] list, int position) {
        ...
        public String getName(){
        	return mService.get().getName();
        }
        public String getImage(){
        	return mService.get().getImage();
        }
        public void enqueue(long [] list , int action) {
        ...
        public int playById(String id){
        	return mService.get().playById(id);
        }
        public int removeById(String id){
        	return mService.get().removeById(id);
        }
    }
