...
public class MyUploadPictureDataAdapter  extends BaseAdapter{
	...
	public MyUploadPictureDataAdapter(Context context, List<DataInfo> result,int page,View.OnLongClickListener mLongClickListener,View.OnClickListener mClickListener) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null ){
			convertView = LayoutInflater.from(context).inflate(
					R.layout.myuploadpicelementlayout, null);
		}
		...
		//System.out.println("MyUploadPictureDataAdapter position = " + position);
		...
		if ((dataInfo.GetIsDir() != null) && (dataInfo.GetIsDir().equals("1"))) {
			...
			if(dataInfo.getThumb()!=null && dataInfo.getThumb().length()>0){
				//System.out.println("thumb = " + dataInfo.getThumb());
				libCloud.DisplayImage(dataInfo.getThumb(), imageView);
				//mImageDownloader2.download(dataInfo.getThumb(), imageView);
			}
		}
