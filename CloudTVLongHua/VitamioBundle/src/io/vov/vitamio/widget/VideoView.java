...
/** Displays a video file. The VideoView class can load images from various sources (such as resources or content providers), takes
 * care of computing its measurement from the video so that it can be used in any layout manager, and provides various display
 * options such as scaling and tinting. VideoView also provide many wrapper methods for {@link io.vov.vitamio.MediaPlayer}, such
 * as {@link #getVideoWidth()}, {@link #setSubShown(boolean)} */
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
	...
	public static final int VIDEO_LAYOUT_DEFINE = 4;
	public static int VIDEO_LAYOUT_WIDTH = 1280;
	public static int VIDEO_LAYOUT_HEIGHT = 674;
	...
	public void setVideoLayout(int layout, float aspectRatio) {
		...
		if (VIDEO_LAYOUT_ORIGIN == layout && mSurfaceWidth < windowWidth && mSurfaceHeight < windowHeight) {
			...
			lp.width = VIDEO_LAYOUT_WIDTH;
			lp.height = VIDEO_LAYOUT_HEIGHT;
		}
		setLayoutParams(lp);
		getHolder().setFixedSize(mSurfaceWidth, mSurfaceHeight);
		Log.d("VIDEO: %dx%dx%f, Surface: %dx%d, LP: %dx%d, Window: %dx%dx%f", mVideoWidth, mVideoHeight, mVideoAspectRatio,
