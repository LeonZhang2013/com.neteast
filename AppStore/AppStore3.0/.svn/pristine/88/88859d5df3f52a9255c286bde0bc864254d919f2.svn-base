...
public class PopWindowActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		slideGesture = new GestureDetector(new ImageGesture(PopWindowActivity.this));
	...
	public void showcomment(Map<String, Object> map) {
		...
		CommentDataAdapter commentadapter = new CommentDataAdapter(PopWindowActivity.this, new ArrayList<Map<String, Object>>());
	...
	private void initcomment() {
		...
		inputword.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if (temp.length() > 80) {
					Toast.makeText(PopWindowActivity.this, "已经超出字数限制", Toast.LENGTH_SHORT).show();
	...
	public void detailDownLoadHandler(View view) {
		...
		if (1 == isRun) {// 1 表示可运行
			...
			if (freeSpace <= fsize) {
				Toast.makeText(PopWindowActivity.this, "SD卡存储空间不足", Toast.LENGTH_SHORT).show();
				return;
			}
	...
	public void onAdviewClick(View view) {
		...
		if (userid == 0) {
			LoginHandler ibh = new LoginHandler(PopWindowActivity.this, map);
	...
	public void closePopWindow(View view) {
		...
		if (view != null) {
			PopWindowActivity.this.setResult(0, resultIntent);
		} else {
			PopWindowActivity.this.setResult(1, resultIntent);
		}
	...
	class commentAsync extends AsyncTask<Object, Object, String> {
		...
		protected void onPostExecute(String result) {
			if (result != null && result.equals("1")) {
				Toast.makeText(PopWindowActivity.this, "评论发表成功", Toast.LENGTH_SHORT).show();
		...
		protected void onPostExecute(String result) {
			if (result != null && result.equals("1")) {
				...
				if (avgscore.equals("5")) {
					Toast.makeText(PopWindowActivity.this, "谢谢配合，但是评论还是失败，网络原因！", Toast.LENGTH_SHORT).show();
				}
				Toast.makeText(PopWindowActivity.this, "评论失败，请选择5星！", Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	class getCommentAsync extends AsyncTask<Object, Object, List<Map<String, Object>>> {
		...
		protected void onPostExecute(List<Map<String, Object>> result) {
			CommentDataAdapter commentadapter = new CommentDataAdapter(PopWindowActivity.this, result);
			mcommentList.setAdapter(commentadapter);
